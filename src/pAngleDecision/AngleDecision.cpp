...
bool AngleDecision::OnNewMail(MOOSMSG_LIST &NewMail)
{
  ...
  for(p=NewMail.begin(); p!=NewMail.end(); p++) {
    ...
    bool   mstr  = msg.IsString();
     ...
     //^_^
     else if(key == "THETA_REPORT") //should be placed before unhandled mail
     {
...
void AngleDecision::Average()
{
	...
	block.theta = (*iter).theta - 90; //atan((block.y2-block.y1)-(block.x2-block.x1))*180/PI;
	angle_point.push_back(block);
