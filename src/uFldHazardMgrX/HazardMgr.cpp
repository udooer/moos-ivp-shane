...
bool HazardMgr::OnNewMail(MOOSMSG_LIST &NewMail)
{
  ...
  for(p=NewMail.begin(); p!=NewMail.end(); p++) {
    ...
    else if(key == "EXCHANGING_DATA_REQUEST"){
      exchangeData(sval);
      askOnlyGiveData(sval);
    }
    else if(key == "OTHERS_HAZARDSET_REPORT"){
        combine(sval);
    }
    else if(key == "RETURN_BACK"){
        exchangeData(sval);
    }
    else 
...
void HazardMgr::registerVariables()
{
  ...
  Register("EXCHANGING_DATA_REQUEST", 0);
  Register("OTHERS_HAZARDSET_REPORT", 0);
  Register("RETURN_BACK", 0);
}
...
void HazardMgr::exchangeData(string dest)
{
  stringstream ss;
  string summary_report = m_hazard_set.getSpec("final_report");
  ss<<"src_node="<<m_report_name<<",dest_node="<<dest<<",var_name=OTHERS_HAZARDSET_REPORT,string_val=\""<<summary_report<<"\"";
  string s;
  ss>>s;ss.clear();
...
void HazardMgr::combine(string others_report)
{
  vector<string> others_data = parseString(others_report, '#');
  vector<string>::iterator i;
  for(i=others_data.begin(); i!=others_data.end();){ 
     XYHazard new_hazard = string2Hazard(*i);
     string hazlabel = new_hazard.getLabel();
     if(hazlabel == "")
       others_data.erase(i++);
     else{
...
void HazardMgr::askOnlyGiveData(string dest)
{
  stringstream ss;
  ss<<"src_node="<<m_report_name<<",dest_node="<<dest<<",var_name=RETURN_BACK,string_val="<<m_report_name;
  string s;
  ss>>s;ss.clear();
  Notify("NODE_MESSAGE_LOCAL",s);
